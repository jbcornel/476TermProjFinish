...
//   // Fetch bookings where the user is the renter, then store those booking objects.
...
//       // Filter bookings for which the user is the renter.
...
//   // Fetch available cars for rent, excluding cars owned by the user.
//   async function fetchAvailableCars() {
//     if (!userId) return;
//     try {
//       const res = await fetch(
//         `/api/cars?available=true&limit=10&search=${encodeURIComponent(searchTerm)}`,
//         {
//           headers: {
//             "Content-Type": "application/json",
//             "x-user-id": userId,
...
//       // Exclude cars that the user owns.
//       const filtered = allAvailable.filter(
//         (car) => car.ownerId !== Number(userId)
//       );
