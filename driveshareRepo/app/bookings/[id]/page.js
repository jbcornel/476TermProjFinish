...
export default function BookingDetailsPage() {
  ...
  //Poll for booking details every 2 seconds.
  useEffect(() => {
    if (!userId) return;
    async function fetchData() {
      await fetchBookingDetails();
  ...
  async function handlePayment() {
    try {
      //send a payment request to the payment server.
  ...
  //Handler for deleting the booking (only allowed if booking is paid & approved and time has passed)
