...
export default function ChatPage() {
  ...
  async function fetchConversation() {
    ...
    try {
      ...
      //Assume server returns messages sorted by timestamp (ascending)
      setMessages(data || []);
    } catch (err) {
  ...
  useEffect(() => {
    ...
    //Fetch the full conversation when the page mounts or partner changes
    fetchConversation();

    //Define the mediator callback for immediate updates
    ...
    //Poll the server every 2 seconds to catch any missed messages.
